...
def test_composite_op_survives_prepare_and_finalize(
    ...
    quantizer = Quantizer(
        ...
        make_quant_config(
            ...
            act_dtype=torch.int8 if quantize_activations else None,
...
class TestCompositeOpIOQuantization:
    ...
    dtype (int8) and the composite config provides a distinct boundary dtype
    (uint8) on the composite's edges. Module config outranks global, so the
    composite boundary must carry the composite dtype while every other quantized
    edge carries the global dtype.
    """

    @staticmethod
    def _composite_act_spec(boundary_dtype: torch.dtype) -> QuantizationSpec:
    ...
    def _config(
        ...
        return make_graph_mode_module_boundary_config(
            module_boundary_dtype=boundary_dtype,
    ...
    def _assert_boundary_quantized(
        ...
        for act_input in tensor_inputs:
            ...
            assert get_quantize_dtype(act_input.args[0]) == boundary_dtype

        users = list(composite.users)
        assert len(users) == 1
        assert is_coreai_quantize(users[0].target)
        assert get_quantize_dtype(users[0]) == boundary_dtype

        composite_dtype_quant = [
            n
            for n in finalized.graph.nodes
            if is_coreai_quantize(n.target) and get_quantize_dtype(n) == boundary_dtype
        ]
    ...
    def test_composite_boundary_input_index_selects_those_args(self, target_by: str) -> None:
        ...
        for index, act_input in enumerate(tensor_inputs):
            if index in quantized_indices:
                ...
                assert input_dtype == boundary_dtype, (
                    f"input {index} was selected by module_input_spec but is quantized as "
                    f"{input_dtype}, expected the composite dtype {boundary_dtype}"
                )
            else:
        ...
        assert consumer_dtype == boundary_dtype, (
            f"the composite's output is quantized as {consumer_dtype}, expected the "
            f"composite dtype {boundary_dtype}"
        )
