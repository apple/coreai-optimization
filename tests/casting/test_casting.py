...
class TestSelectiveOpSkipping:
    ...
    def test_ignored_op_prevents_activation_overflow(self):
        ...
        cast_fp32_to_fp16(ep_skipped, ignored_ops=["exp", "log1p"])
    ...
    @pytest.mark.parametrize(
        "spec",
        [
            pytest.param([torch.ops.aten.exp.default], id="op_overload"),
            pytest.param([torch.ops.aten.exp], id="op_overload_packet"),
            pytest.param([torch.exp], id="callable"),
            pytest.param(["exp"], id="str_name"),
            pytest.param(["aten.exp"], id="qualified_str"),
        ],
    )
    ...
    def test_target_specific_node_name(self):
        """Targeting a specific node name preserves only that instance in FP32."""
        ...
        # Ignore only the second exp node
        cast_fp32_to_fp16(ep, ignored_ops=[second_node_name])
    ...
    def test_consecutive_ignored_ops_eliminate_redundant_casts(self):
        ...
        cast_fp32_to_fp16(ep, ignored_ops=["exp", "log1p"])
    ...
    def test_ignored_creation_op(self):
        ...
        cast_fp32_to_fp16(ep, ignored_ops=["full_like"])
    ...
    def test_cast_to_16_bit_precision_forwards_ignored_ops(self):
        ...
        cast_to_16_bit_precision(ep, ignored_ops=["exp", "log1p"])
