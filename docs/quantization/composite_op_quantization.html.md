...
It returns a list of torch exported programs corresponding to all the composite op sub-modules that were *patched* for externalization by `_patch_model_for_externalization`.
...
#### WARNING
The externalization APIs used above, `_patch_model_for_externalization` and `_subexport_and_restore` in coreai-torch are currently experimental.
